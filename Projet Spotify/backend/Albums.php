...
class Albums extends Db {
    ...
    static function getAllAlbums() {
        $db = Db::dbConnect();
        $query = $db->prepare("SELECT al.id AS 'album_id', al.name AS 'album_name', al.description AS 'album_description', al.cover AS 'album_cover', al.popularity AS 'album_popularity', al.release_date AS 'album_release_date', ar.name AS 'artist_name' FROM albums al INNER JOIN artists ar ON al.artist_id = ar.id");
        $query->execute();
        $results = $query->fetchAll(PDO::FETCH_ASSOC);
        return json_encode($results);
    }
